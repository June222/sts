...
var assembleDo = function () {
  /** NOTE:
   * 1. current url이 국회 입법 예고 입법 페이지인 경우
   * 2. current url이 vforkorea 인 경우
   * 3. 그 외의 경우
   */
  ...
  if (regex_pa.test(cururl)) {
    ...
    /**
     * current url이 vforkorea 라면
     * 입법 사이트 로그인 후
     * 의견 종류 고르고
     * 한번에 열 입법 탭의 개수를 고르고
     * 설정에 따라 현재 화면에 load된 모든 입법(tr)에 대하여
     * 체크 박스 확인 후 작업 List에 추가하여 List의 내용만 작업을 함
     */
    if (!isLogin) {
      isLogin = confirm(
